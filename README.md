...
最新家族级前沿清单把目录身份与外部历史证据合并为 21 个可追踪家族标识：16 个已有测量
或主线证据，3 个没有有效测量但已有自动执行失败记录，1 个没有可用参考绑定，另 1 个只
有外部证据而没有目录位置。因此当前没有可以不经修复就直接安全重跑的“新族”目标；下一步
应修复参考绑定或补充参考适配器，再按同一入口测量。该结论来自静态身份、支持状态和历史
执行记录，不读取任何数值结果，也不把失败族写成负例。
