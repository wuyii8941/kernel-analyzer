#!/usr/bin/env python
"""
Arm E: compile path with fork-aware UNCLIPPED fallback.

For tokens whose clipping margin is smaller than the empirical
compile-eager delta threshold, force the unclipped objective.
This preserves gradient signal (unlike detach) and matches eager
behavior in the typical fork case where compile incorrectly clips
a token that eager would not clip.
"""
...
def fork_aware_unclipped_loss(torch, logps, old, advantages, eps: float, margin_threshold: float):
    ...
    objectives = torch.where(near_boundary, unclipped, objectives)

    return -objectives.mean(), ratio, clipped_ratio, near_boundary


def main() -> None:
    parser = argparse.ArgumentParser(description="Arm E: compile with fork-aware unclipped fallback.")
    ...
    for step in range(1, args.steps + 1):
        ...
        loss, _, _, near_mask = fork_aware_unclipped_loss(
