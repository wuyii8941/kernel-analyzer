#!/usr/bin/env python3
"""Bind checked embedding lookup outputs to recorded parameter paths."""

import argparse
from pathlib import Path

from scripts import bind_residual_rms_forward as path_binding
from scripts import scan_embedding_lookup_sources as source_scan
from scripts.run_numerical_coverage import read, save, sha
from kernel_analyzer import embedding_lookup_source, embedding_lookup_reference
...
def bind(tasks, contracts, mapping):
    ...
    for case in result["cases"]:
        ...
        case["case_id"] = case["case_id"][:-len(suffix)] + "-embedding-lookup"
        case["reference_method"] = "EMBEDDING_LOOKUP_COMMON_INPUT"
    result["schema"] = "embedding-lookup-forward-bound-plan-v1"
    return result
...
def main():
    ...
    contracts = {row["symbol"]: row["contract"] for row in rows
                 if row["status"] == "SOURCE_CHECKED"}
    result = bind(read(args.tasks)["rows"], contracts, mapping)
    result["source_scan"] = rows
    dependencies = [
        args.source, args.tasks, args.mapping, Path(__file__), Path(path_binding.__file__),
        Path(source_scan.__file__), Path(source_scan.scan_recurrence_sources.__file__),
        Path(embedding_lookup_source.__file__), Path(embedding_lookup_reference.__file__),
    ]
    result["source_sha256"] = {str(path.resolve()): sha(path) for path in dependencies}
    save(args.output, result)
    print({"bound": len(result["cases"]), "unresolved": len(result["unresolved"]),
