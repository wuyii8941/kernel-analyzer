#!/usr/bin/env python3
"""Discover pure Triton embedding lookups using their complete source body."""
...
from kernel_analyzer import embedding_lookup_source


def scan(source):
    return scan_recurrence_sources.scan(source, checker=embedding_lookup_source.check_source)
...
def main():
    ...
    for path in args.source:
        ...
        sources.append({"path": str(path.resolve()), "sha256": sha(path), "rows": rows})
        print({"source": str(path), "definitions": len(rows),
    ...
    dependencies = [Path(__file__), Path(scan_recurrence_sources.__file__),
                    Path(embedding_lookup_source.__file__)]
    save(args.output, {
        "schema": "embedding-lookup-source-scan-v1",
        "sources": sources,
