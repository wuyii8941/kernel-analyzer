...
def main() -> None:
    ...
    rows = [
        ...
        ("liger_fused_linear_ce_dw_accumulation", "FP32 addition-order source plus nonzero profile means reproduced on disjoint length-64 and length-256 confirmation banks; a real length-64 Kahan intervention removes only part of the gradient difference and does not reduce update energy", "actual-write generality or causal sufficiency for loss", "original-coordinate writes or a predeclared longer paired loss comparison; Kahan is not a certified update repair"),
