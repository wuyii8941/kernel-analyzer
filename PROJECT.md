...
实际接入说明只维护在 [算子接入与自动化边界](docs/system.md)。已审核家族可自动
运行和分析；任意源码仍需参考、输入及必要的训练接入，不能宣称零适配支持全部算子。
下一轮优先研究同精度实现选择的 bias 来源，不预设低精度或 QK channel 为主案例；
已有量化案例、对照和训练结果保留，不通过改变叙事提高证据等级。

数学推导、三阶段测量、等价范围检查和训练验证各回答不同问题。阴性、未决和仅有
部分机制证据的记录也是有效分析结果，不能冒充已经证明训练收益的案例。
四目标的标准与未完成项统一见[当前主线](docs/current_mainline.md)与
[主张账本](docs/claims.md)；
