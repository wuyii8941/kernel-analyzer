...
2. **完整误差标准对比**：三个新的 Gemma 行已经重新保存输出、梯度和更新原始值，并完成了输出/梯度的 ULP 与 `rtol/atol` 摘要；但历史回放仍没有原始位级 operand，Gemma 更新也没有保存 candidate/repair 的独立更新对，所以全池完整对比仍未完成。
   采集器现在支持在同一冻结运行版本中保存 candidate/reference 输出对；本轮旧运行版本无法通过结构身份检查，因此没有把失败重放当作数据。
