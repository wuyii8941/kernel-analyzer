...
仓库中有 **23 个唯一主矩阵 case ID、29 条矩阵记录**。另外有 **11 个历史上曾被认为可能有 bias 的候选**；合并后的长程审计表共有 **26 行**。这三个数字分别是覆盖分母、历史候选分母和审计行数，不能互相替代。

在这 11 个历史候选中，当前有 **4 个最终案例**：3 个直接作用长程成立（Liger fused CE、Phi `lm_head dX`、Qwen `lm_head dX`），以及 1 个反馈维持型长程案例（Qwen3-VL SiLU）。最终案例要求长程方向或反馈分离成立，并且配对训练中已经观察到参数和 loss gap；不要求两条轨迹收敛到不同的最终 loss。完整逐行表见 [`all_bias_long_horizon_audit.md`](all_bias_long_horizon_audit.md)。
...
| Gemma4 RMSNorm/反馈区域 | — | — | 冻结运行包与当前编译环境不一致，拒绝重放 |
| layer-23 attention state | — | — | 历史实现无法精确重放，拒绝判断 |
