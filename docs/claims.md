...
unique matrix IDs, 11 historical candidates, and 26 rows. Under the warm-state
4096-step review, Liger, Phi and Qwen `lm_head dX` retain robust direct
direction; Qwen3-VL SiLU shows feedback-sustained separation with a paired loss
gap. Mamba, saved-P and both `v_proj` rows do not; layer-23, Gemma4 and
DeepSeek `dV` remain unresolved. Historical six- and eight-row registries are
...
2. Six project cases pass the historical strict F+B/repair/carrier/trajectory
   ...
   warm-state 4096-step review, Liger, Phi and Qwen retain direct direction;
   SiLU is feedback-sustained with a paired loss gap; Mamba, saved-P and both
   `v_proj` rows do not; layer-23, Gemma4 and DeepSeek `dV` remain unresolved.
