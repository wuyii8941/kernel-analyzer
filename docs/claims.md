...
Evidence snapshot: current organized mainline (2026-08-25). The historical
stateless-SGD headline contains three short-horizon operator/backward records.
The corrected common-AdamW comparison is now treated as a 16/32-step screen,
not the final long-horizon label. The merged long-horizon audit contains 23
unique matrix IDs, 11 historical candidates, and 28 rows. Under the warm-state
4096-step review, Liger, Phi, Qwen, Llama and Ministral `lm_head dX`/fused CE
records retain robust direct direction; Qwen3-VL SiLU shows feedback-sustained
separation with a paired loss gap. Mamba, saved-P and both `v_proj` rows do
not; layer-23, Gemma4 and DeepSeek `dV` remain unresolved. Historical six- and
eight-row registries are retained only as audit history.
...
| Horizon comparisons are valid. | Use the same horizon or compare each path to its horizon-matched empirical null; do not compare raw `A` values across different ceilings. | `results/property/bias_property_search/short_screen_protocol_v3.json`; `results/property/declared_persistent_4096/summary.md` | `SUPPORTED_WITH_SCOPE`. The Oracle uses 16/32 steps for screening and mechanism localization. Current long-horizon labels use 4096 steps plus late rolling windows. Cross-horizon raw magnitudes are not ranked directly. |
...
| Three bounded records support the historical stateless-SGD direct operator/backward short-horizon headline. | A valid one-operator repair reaches a real parameter and the resulting SGD update difference has direction on the declared ordered trajectory. | `results/property/joint_bias_formation_v1/source_persistence_reclassification.json`; `results/property/joint_bias_formation_v1/headline_case_evidence_scope_v1.json`; `case.md` | `SUPPORTED_AS_SHORT_HORIZON_HISTORY`: Liger fused CE, Phi `lm_head dX`, and the Qwen `lm_head dX` family. The exact Qwen seq128 cold-start AdamW rerun is canceling at 32 steps, while its later warm-state 4096-step review is robustly directional. Short and long protocols must not be collapsed into one label. |
| Declared cases retain direct direction over 4096 warm-state steps. | Warm up 128 steps, measure 4096 same-state candidate-minus-repair updates, compare to the case's sign-flip null, and inspect late rolling windows. | `results/property/declared_persistent_4096/all_bias_case_audit.json`; per-case JSON under `results/property/declared_persistent_4096/` | `SUPPORTED_CASE_LEVEL`: Liger, Phi, Qwen, Llama and Ministral `lm_head dX`/fused CE rows are robust direct cases; Qwen3-VL SiLU is feedback-sustained with a paired loss gap; Mamba, saved-P and both `v_proj` rows are not robust; layer-23, Gemma4 and DeepSeek `dV` remain unresolved. A separate queue now contains 12 result-blind 32-step consequence candidates; they are unresolved until their long replay completes and are not negatives. This is not loss convergence or full-parameter training. |
...
| The Oracle has measured short-screen ranking behavior. | Report flagged fraction and retrospective ordering under one short-horizon optimizer protocol; do not interpret a non-escalation as a long-horizon negative. | `results/property/joint_bias_formation_v1/oracle_repair_v3/same_optimizer_oracle_v3.json`; `docs/oracle_repair_v3.md`; `results/property/declared_persistent_4096/summary.md` | `SUPPORTED_FOR_RETROSPECTIVE_15_ROWS_AS_TRIAGE`. The earlier 5/15 flag rate and same-protocol ranking remain valid short-screen measurements. Qwen shows that a cold-start non-escalation cannot be transferred to a newer warm-state long-horizon protocol. Because state stream and horizon differ, this is a scope limitation rather than a same-protocol false-negative estimate. |
...
2. Six project cases pass the historical strict F+B/repair/carrier/trajectory
   registry. Three support the historical stateless-SGD short-horizon
   headline. The common AdamW 16/32-step table is a screen. In the current
   warm-state 4096-step review, five direct cases are confirmed (Liger, Phi,
   Qwen, Llama and Ministral) and SiLU is feedback-sustained with a paired
   loss gap; Mamba, saved-P and both `v_proj` rows do not; layer-23, Gemma4
   and DeepSeek `dV` remain unresolved.
...
4. A short sequence of local parameter-update differences is useful for
   fail-closed prioritization on the corrected 15-row same-AdamW evaluation
   set, but Qwen proves that it cannot replace long-horizon review.
5. Feedback-sustained drift is a distinct case-level regime demonstrated by
   Gemma’s Adam-state intervention.
6. The workflow is `16/32-step triage -> 4096-step review`; neither stage is a
   safety certificate or a validated universal classifier, and 4096 direct
   direction is not a converged-loss claim.
...
历史上所有曾被认为可能有 bias 的候选，统一见
`docs/all_bias_long_horizon_audit.md` 及其 JSON 原始表。最终案例必须同时有：

1. 直接源案例：4096 步同状态直接更新差异仍然稳定有方向；或
2. 反馈维持型案例：4096 步实际反馈分离仍然存在；并且
3. 配对训练已经观察到参数和 loss 分叉。

这不要求两条轨迹收敛到不同的最终 loss。反馈维持型案例单独标记，不能写成直接算子 bias。短程 16/32 步结果只负责筛查和解释形成位置。
