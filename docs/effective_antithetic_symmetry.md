...
### Fixed-condition source-debias confirmations

The global carrier is not used as the source-bias definition.  Qwen seq64 and
seq128 `v_proj` were each held at 16 fixed training conditions while only the
source-aligned stochastic materialization changed.  For both cases, the repair
local residual is centered in 16/16 conditions, while the deterministic
candidate-minus-repair effect is biased at the local output, actual parameter
...
the candidate local/Adam effects are biased in 16/16, but real gradient/SGD is
biased in 13/16 and unresolved in three.  Thus source asymmetry exists without
a uniformly certified linear backward transport across every condition.

Layer-23 supplies the complementary representation boundary.  Direct BF16
reflection around the eager `S_bwd` reference is not always representable.  A
fixed nearest-BF16 projection gives exact pairs and shams in 16/16, but falls
below the frozen 90% natural-source-fidelity gate in some states.  Its exact
nearby pairs show F+B response-even ratios of 0.029--0.166 and zero-moment
AdamW ratios of 0.672--0.725.  These measurements support response
rectification in the projected neighborhood, but do not close a
marginal-preserving mechanism for the natural layer-23 source.
...
> the real F+B/optimizer response has a nonzero even component.  Liger, Phi,
> and the Qwen64/128 fixed-state source repairs support the event/pairing
> channel; the exact saved-P `+delta_g/-delta_g`
...
result, not yet a universal predictor for unseen operators.  Mamba and
layer-23 remain explicit partial boundaries rather than being counted as
additional matched positives.  Saved-P remains a
