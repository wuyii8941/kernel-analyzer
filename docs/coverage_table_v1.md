...
| 6 | 当前长程最终案例（5 个直接方向 + 1 个反馈维持） | 当前长程审计主案例计数；不等于六种独立实现机制 |
...
这段 sample-completion 快照保留作历史审计，不能覆盖当前长程审计。当前统一的长程口径见 [`all_bias_long_horizon_audit.md`](all_bias_long_horizon_audit.md)：23 个唯一主矩阵 ID、11 个历史候选加 2 个同族复现行、28 行合并审计和 6 个最终案例。不同协议仍不能把记录简单相加。
