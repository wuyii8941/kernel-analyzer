...
与原记录一致；8份开启补偿训练的全部1024步训练loss均复现历史结果。
...
分析文件、原始文件、冻结源码和生产分析结果之间的记录一致。这不是由保存的
...
按原始数据记录和stage去重，得到376组case/capture：82组local allclose通过、145组
...
来自补偿，因此追加下述同路径训练。两轮短对照协议均在测量前保存。第一轮快照
在后续修改后按原内容恢复。
...
376组baseline已进一步核对423份报告、原坐标统计、已记录tolerance
...
冻结依赖并得到完全相同的差值和区间，状态为 `VERIFIED`。
