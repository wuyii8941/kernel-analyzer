...
| 4096 步长程复核 | `COMPLETE_WITH_FAIL_CLOSED_ABSTENTION` | Liger、Phi、Qwen、Llama、Ministral、Mamba、saved-P、两条 `v_proj` 已有直接审计；SiLU 为反馈维持型长程结果；layer-23 fail-closed abstain | Gemma4/DeepSeek 等仍按逐行审计表保持未决；Mamba 的直接审计未超过自身随机基线，但 live 配对 loss 审计已完成并观察到分叉；完成项见 `results/property/declared_persistent_4096/summary.md` 与 `docs/all_bias_long_horizon_audit.md` |
