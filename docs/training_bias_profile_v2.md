...
> 测量方法记录，不是当前科学主线的替代品。具体成因、干预和 loss 连接见
> [案例地图](case_evidence_map.md)。这里的五例每个输入都从零 moments 单独测量，
> 不是从零开始后连续演化 32 步；原始数值与判定规则保留。
...
随后完成的统一 16 项冻结验证中，15 项得到有效测量：9 项确认 update 缩小。修正后
增加完整 Gram 或随机摘要上的 update difference 总体范围检查，原标签为 3 项通过固定输入 update
等价、3 项总体 update 比例超过范围、1 项无法判断。两个 Mamba seq128 案例的三个
冻结随机摘要均为零，但没有保留原向量，因此不称为逐位相同。原三方向证书中的 Mamba
seq64 因三个摘要中的最大比例为 `2.142%` 被撤销等价标签；两个 Phi 方向结果仍未
确认，但总体 update 比例也已超过范围。Mamba seq256 因实际 backward 图不一致保留
无法判断。
Qwen 原冻结位置与 Mamba seq64/128 均使用原案例完成，没有用替代案例补位。

完整 update 范围是在结果揭示后为修复方向盲区而加入，因此不能冒充事前冻结；旧
三方向标签只保留为历史记录。当前修正只签发固定输入集合中的 update 等价，不外推到
随机训练状态或训练质量。大向量所用三个摘要比例的最大值未被证明是原向量的上界；
因此这些历史标签还必须保留“摘要估计”的测量范围，不能统称原坐标全空间保证。
