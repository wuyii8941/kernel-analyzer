...
> 版本定位（2026-09-05）：按各批冻结时间记录。早期运行未决和后续修复保留先后，不按最新状态回写旧前瞻结果。
...
这里保留等价 v2 重新分析的机器标签；大向量使用三个随机摘要，其最大比例不是
已证明的原坐标能量上界。下文“通过范围”须按该测量方式理解，不是完整训练安全。
该方法修订发生在结果揭示后，也不改变前两批实验各自的前瞻范围。
