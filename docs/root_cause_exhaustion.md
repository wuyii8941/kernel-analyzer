...
这是针对当前保留证据的缺口表；当前 12 个去重问题组、16 个冻结 benchmark 案例和
17 个算子族的逐项状态，以[当前根因闭环账本](root_cause_closure_current.md)为准。
本页的详细表格保留最初七个核心问题组的推导边界，不再单独维护问题组总数。
“耗尽”只表示现有记录不能再推出更强的唯一根因；剩余升级都需要新的区分性观测，
不是把缺失观测改写成阴性结果。
