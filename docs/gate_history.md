...
| 2026-08-31 | expanded exact-target long-run audit | Operator-scan and legacy coherent candidates were added without changing the evidence hierarchy | The machine audit now contains 301 rows. It reports 43 rows with long-run bias evidence plus paired loss separation (3 late-window direct, 8 aggregate direct without exported late windows, 32 feedback-sustained), but only 4 rows currently have explicit late-window confirmation. The broader outcome-relevant count is 105 and includes persistence-unmeasured candidates; it is not a final persistent-case count. |
