...
对超过固定摘要容量的大参数张量，自动入口只保留上述原坐标标量，并在结果中标记
方向摘要未生成；这类记录仍可进行 Q 的固定集合判断，但不输出方向诊断。
...
[总体超界比例检验](population_exceedance_inference.md)。AdamW8bit 已在一个固定 checkpoint
经验 token 总体上完成 32 条独立有放回 gradient histories 的真实应用；该结果只覆盖
声明的超界比例，不覆盖平均 Q、自然递推训练状态或其他 checkpoint。
...
这些训练流通过“不放回候选＋最小起点间距”选取，后一个起点是否入选依赖已经选中的
起点，因此不构成严格 iid 抽样。现有配对 t 区间保留为这组冻结训练流上的条件性近似，
不升级为对候选 token 总体的严格 iid 推断。随后已经在修改与终点冻结后，从与开发
范围分离的声明总体独立有放回抽取 8 条完整训练流；8 对均完成，OFF−ON 均值
`+0.0282858`，95% 配对 t 区间 `[+0.0157405,+0.0408311]`。该结果补上新训练流
确认，但仍条件于固定 checkpoint、评估集、起点总体和配对差值正态假设。

若某条件在终点前出现协议定义的非有限 loss 或参数，最终有限 loss 不存在，不能用
无穷或人为大值进入 t 区间。汇总按“数据流＋条件”区分完整、训练数值失败、执行失败和
未完成；重试不增加样本数。双方均完成子集只能作条件性描述。本轮结果揭示后增加的
失败率分析作补充描述，下一轮推断前另行冻结失败端点。完整说明见
