...
精确 AOT 端点和保守结构签名为每个位置记录计算组。分类覆盖不等于支持；去重后的
有效测量仍为 520。已有通用 AOT 重放和外部矩阵乘法相同输入重算被自动连接后，
32,239 个位置具备 reference 与训练参数绑定，进入不同家族优先的待测队列，其中
16,282 个是 Triton。AOT 重放只支持闭合区域作用测量，不升级为单 kernel 成因。
清单、表和命令见[全部已观测 kernel 清单](observed_kernel_catalog_v2.md)。
