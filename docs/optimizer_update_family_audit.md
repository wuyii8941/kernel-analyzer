...
这支持“跨步量化残差 → 修改 → 固定训练设置中的loss改善”的研究路径，但还不能把
旧默认编译与新补偿实现之间全部差异唯一归因于补偿。后续同路径重放已经完成：
8/8组开启补偿loss更低，均值0.0220243，95%配对区间[0.0138561,0.0301924]；
...
旧训练流采用不放回候选与最小间距筛选，不能提供严格 iid 抽样。为避免把同数据归因
重放当成新的总体确认，后续在修改、1024 步终点、固定评估集和 +0.01 门槛冻结后，
从与开发范围分离的起点总体 `[500000,700000)` 独立有放回抽取 8 个完整 token 流。
实际抽样起点全部保留，不按重叠、差值或运行结果重抽。

OFF 与 ON 的 16 份训练全部有限完成。逐流 OFF−ON 为
`[0.02443,0.01524,0.04539,0.03170,0.02829,0.04994,0.00343,0.02786]`；
8/8 为正，均值 `+0.0282858`，95% 配对 t 区间
`[+0.0157405,+0.0408311]`，整个区间超过预声明 +0.01 门槛，机器独立复算为
`VERIFIED`。这使“补偿读取本身 → 声明训练分布中的平均 loss 改善”不再只依赖已见
数据流重放。

该区间仍依赖独立起点抽样和配对差值正态假设；固定 checkpoint、评估集、数据集和
optimizer 设置构成结论边界。第 7 条差值仅 `+0.00343`，所以不能声称每条训练流都
超过实际幅度门槛。原始协议、16 个运行记录、汇总和核验位于
