...
## 2026-09-13/14 结构训练失败记录

`result_analysis_v3/structured_training` 的冻结运行器遇到非有限 loss 时直接抛出异常，
不会自动继续后续任务。三版 coordinate-roll 恢复脚本只负责调用同一冻结
`run_condition` 并保存终态；前两版分别暴露 CUDA 不可用和仓库模块路径缺失，相关
执行失败均保留，第三版在环境正确后得到 8/8 数值失败。重试不增加样本数。

key-only 的第 8 条任务首次在第 936 步抛出异常但未落盘；通用单任务捕获器随后用同一
冻结任务重放，并在相同步骤复现非有限 loss。最初恢复元数据把首次尝试简称为
`interrupted`，追加 clarification 文件明确它实际由数值异常终止。最终失败感知汇总按
“数据流＋条件”去重，完整结果优先于较早环境失败，数值失败不填入 final-loss 区间。
