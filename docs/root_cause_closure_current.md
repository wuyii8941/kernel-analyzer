...
| `mm_gemm_output_and_accumulation` | CONDITIONAL_SOURCE_ISOLATION_NOT_UNIVERSAL_MM_ROOT；kernel arithmetic residual and final output rounding are separable for selected fixed-input comparisons | historical trajectories separate, but robust cross-state direction is case-dependent | same-operands factorial separating accumulation arithmetic, final cast, and their interaction on an independent state bank |
