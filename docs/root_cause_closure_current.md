...
当前共 12 个科学问题组；其中 1 个连到声明的 material loss improvement，7 个至少完成了局部来源闭环，2 个是阴性控制；另保留 2 个测量覆盖集合。其余开放分支明确列出，不用‘有差异’代替根因。
...
| `mm_gemm_output_and_accumulation` | CASE_SPECIFIC_SOURCE_AND_CONDITIONAL_F_B_EFFECT_CLOSED_NATURAL_GENERALIZATION_OPEN；case-specific finite-precision sources: output rounding only in Qwen128, kernel plus output rounding in Qwen64/Mamba, and kernel arithmetic in Phi | historical trajectories separate, but robust cross-state direction is case-dependent | an absolute high-precision downstream reference or independently sampled natural-state confirmation is needed for a stronger cross-case MM or population-bias claim; the current conditional result is already closed for the named same-input branches |
