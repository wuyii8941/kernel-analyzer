...
The existing repository has exact antithetic response replays for saved-P and SiLU, a composite transport intervention for Phi, and source-event/chunk-geometry evidence for Liger. A host-GPU Liger 32-state rerun completed, but its confirmation population remained unresolved under the frozen gate. The 24-state chunk intervention confirmed BF16-specific directional geometry (24/24 versus FP32 13/11). The separate RN→SR default-residual run was diffusive in its RN arm (A=0.942 versus SR=0.975/1.001), so it is `INCONCLUSIVE_NO_POSITIVE_BASELINE`, not evidence that a confirmed persistent source survived SR. A half-learning-rate Phi trajectory still passed SEUP with local accumulation 2.37477e-5, feedback 2.04859e-6, and recurrence residual 1.01e-8; a fixed-error alternate-checkpoint probe gave drift/direct-sum ratio 1.0104. The latter is positive propagation closure: direct accumulated error predicts the observed drift to within 1.04%, with no additional feedback amplification in that probe. The deterministic 12-row screen-negative audit is screen-level only; no full trajectory label is imputed.
